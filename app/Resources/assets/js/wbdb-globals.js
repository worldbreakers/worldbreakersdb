...
window.find_identity = function find_identity() {
  Identity = WBDB.data.cards
    .find({ indeck: { $gt: 0 }, type_code: "identity" })
    .pop();
};

window.update_deck = function update_deck(options) {
...
window.update_charts = function update_charts() {
  if (!("charts" in WBDB)) {
    WBDB.charts = create_charts();
  }

  if (WBDB.charts.cost) {
    var costData = repartitionByCost();
    WBDB.charts.cost.data = costData;
    WBDB.charts.cost.update();
  }

  if (WBDB.charts.cardCount) {
    var cardCountData = cardCountGraphData();
    WBDB.charts.cardCount.data = cardCountData;
    WBDB.charts.cardCount.update();
  }
};
...
function check_decksize() {
  ...
  $("#cardcount")
    .html(WBDB.DeckSize + " cards")
    [WBDB.DeckSize !== 30 ? "addClass" : "removeClass"]("text-danger");
}
  ...
  cards_used.forEach(function (oldCode) {
    ...
    if (newCode) {
      ...
      if (update) {
        ...
        WBDB.data.cards.updateById(newCode, {
          indeck: quantity,
        });
        WBDB.data.cards.updateById(oldCode, {
          indeck: 0,
        });
...
window.export_bbcode = function export_bbcode() {
...
window.export_markdown = function export_markdown() {
...
window.export_plaintext = function export_plaintext() {
...
function build_tts(deck) {
  ...
  return Promise.all(images).then(function (images) {
...
window.download_tts = function download_tts(deck) {
