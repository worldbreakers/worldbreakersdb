(function (suggestions, $) {
  ...
  suggestions.refresh = function () {
    suggestions.number = parseInt(
      NRDB.settings.getItem("show-suggestions"),
      10
    );
  };
  ...
  suggestions.show = function () {
    ...
    for (var i = 0; i < suggestions.current.length; i++) {
      ...
      var cards = NRDB.data.cards.find(
        { title: suggestions.current[i].title },
        { $orderBy: { code: -1 } }
      );
      var card = cards[0];
      if (Filters.pack_code.indexOf(card.pack_code) > -1) {
        ...
        div.on(
          "click",
          "button.close",
          suggestions.exclude.bind(this, card.title)
        );
        tbody.append(div);
  ...
  suggestions.div = function (card) {
    ...
    var div = $(
      '<tr class="card-container" data-index="' +
        ...
        card.title +
        '</a></td><td></td><td class="type" title="' +
        card.type.name +
