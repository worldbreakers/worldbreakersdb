...
function check_decksize() {
  ...
  $("#cardcount")
    .html(DeckSize + " cards")
    [DeckSize !== 30 ? "addClass" : "removeClass"]("text-danger");
}
    ...
    function (i, line) {
      switch ($(line).prop("tagName")) {
        ...
        default:
          ...
          var card = NRDB.data.cards.findById(
...
function build_markdown(deck) {
  ...
  if (typeof Decklist != "undefined" && Decklist != null) {
    ...
    lines.push(
      "Deck built on [WorldbreakersDB](" + NRDB.worldbreakersdb_url + ")."
    );
  }
  return lines;
}
...
function cardCountGraphData() {
  ...
  var guilds = NRDB.data.factions.find(
    { code: { $ne: "neutral" } },
    { $orderBy: { name: 1 } }
  );
